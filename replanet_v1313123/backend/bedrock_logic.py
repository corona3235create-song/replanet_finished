...
from backend.routes.credits import get_garden_status # get_garden_status 임포트
...
async def chatbot_endpoint(request: ChatRequest):
    ...
    You must choose one of the following actions and respond only in JSON format. Do not add any other explanations.

    1. "get_user_carbon_data": Choose this when the user asks about their own carbon reduction amount.
       - Example: {{"action": "get_user_carbon_data", "query": "내가 절약한 탄소량은?"}}

    2. "get_user_points": Choose this when the user asks about their own accumulated points.
       - Example: {{"action": "get_user_points", "query": "내가 모은 포인트는?"}}

    3. "get_user_garden_level": Choose this when the user asks about their own garden level.
       - Example: {{"action": "get_user_garden_level", "query": "내 정원 레벨은?"}}

    4. "get_user_challenge_progress": Choose this when the user asks about their own challenge progress.
       - Example: {{"action": "get_user_challenge_progress", "query": "챌린지 진행 상황 알려줘"}}

    5. "knowledge_base_search": Choose this when the user's question is likely to be answered by a private knowledge base, containing specific information about people, projects, or internal documents. Specific topics like 'carbon reduction' or 'recycling' can also belong here.
    ...
    6. "general_search": Choose this when the user's question requires up-to-date information or general knowledge that is unlikely to be in a private knowledge base, such as 'today's weather' or 'news about celebrities'.
       - Example: {{"action": "general_search", "query": "today's weather in Seoul"}}

    7. "recommend_challenge": Choose this when the user explicitly asks for a challenge recommendation or expresses a desire to start a new eco-friendly challenge.
    ...
    8. "get_carbon_reduction_tip": Choose this when the user asks for tips on reducing carbon emissions or eco-friendly practices.
    ...
    9. "get_goal_strategy": Choose this when the user asks for strategies to achieve their eco-friendly goals or improve their progress.
    ...
    10. "direct_answer": Choose this for simple greetings or small talk, like "Hello", "Thank you", or "Who are you?".
       - Example: {{"action": "direct_answer", "answer": "Hello! I am an AI chatbot here to answer your questions. Ask me anything!"}}

    11. "detect_activity_and_suggest_challenge": Choose this when the user mentions a specific eco-friendly action they have taken, like "I walked to work" or "I took the bus today". This is different from asking for a recommendation.
